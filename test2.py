...
sims = averager(sims)
plt.plot(range(200), sims, color="red")

sims = []
print("new prob: ", 0.5)
SIR.population.trace_prob = 0.5
for _ in range(reps):
    print(_)
    data = SIR.run(list(range(200)))
    sims.append(data["R"])
sims = averager(sims)
plt.plot(range(200), sims, color="orange")

sims = []
print("new prob: ", 1)
SIR.population.trace_prob = 1
for _ in range(reps):
    print(_)
    data = SIR.run(list(range(200)))
    sims.append(data["R"])
sims = averager(sims)
plt.plot(range(200), sims, color="green")


plt.xlabel("t")
plt.ylabel("R")
plt.title("R(t) where red is p=0, orange is p=0.5, green is p=1")
plt.show()